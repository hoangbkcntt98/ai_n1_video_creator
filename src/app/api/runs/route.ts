...
export async function POST(request: Request) {
  try {
    const body = await request.json() as { action?: string; patternId?: unknown; patternName?: unknown };
    if (body.action === "create_next") {
      const run = await startGeneration({});
      return Response.json({ run }, { status: 202 });
    }
    if (body.action === "generate_pattern") {
      const run = await startGeneration({ patternId: Number(body.patternId), patternName: typeof body.patternName === "string" ? body.patternName : "" });
      return Response.json({ run }, { status: 202 });
    }
