...
function findQuestionAudio() {
  const audioPath = path.join(__dirname, "..", "audio", "question.wav");
  return fs.statSync(audioPath, { throwIfNoEntry: false })?.isFile() ? audioPath : null;
}
...
function overlayArguments(overlays) {
  if (!overlays.length) return { filterArgs: ["-vf", "pad=ceil(iw/2)*2:ceil(ih/2)*2"], mapArgs: [] };
  ...
  return { filterArgs: ["-filter_complex", filters.join(";")], mapArgs: ["-map", "[out]"] };
}
...
async function main() {
  ...
  const questionAudio = option("--question-audio", findQuestionAudio() || "");
  ...
  try {
    ...
    await execFileAsync(process.env.FFMPEG_PATH || "ffmpeg", [
      ...
      ...(() => { const { filterArgs } = overlayArguments(overlays); return filterArgs; })(),
      ...(questionAudio && fs.statSync(questionAudio, { throwIfNoEntry: false })?.isFile() 
        ? ["-i", questionAudio] 
        : []),
      ...(() => { const { mapArgs } = overlayArguments(overlays); return mapArgs; })(),
      ...(questionAudio && fs.statSync(questionAudio, { throwIfNoEntry: false })?.isFile() 
        ? ["-map", `${overlays.length + 1}:a`, "-af", `apad=whole_dur=${(duration + highlightDuration).toFixed(3)}`] 
        : []),
      ...(questionAudio && fs.statSync(questionAudio, { throwIfNoEntry: false })?.isFile() 
        ? ["-c:a", "aac", "-b:a", "192k"] 
        : ["-an"]),
